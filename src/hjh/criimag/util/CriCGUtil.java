...
public class CriCGUtil {
	...
	public static void setGroundFromView(View view, AttributeSet attrs) {
		...
		for (int i = 0; i < N; i++) {
			...
			case R.styleable.cri_criView_cri_groundnormal:
				GradientDrawable grade = null;
				// 渐变色
				gradColor = a
						.getString(R.styleable.cri_criView_cri_normalgradientcolors);
				if (gradColor != null) {
				...
				// 圆角
				radiusInt = a.getString(R.styleable.cri_criView_cri_cornersradius);
				if (radiusInt != null) {
					// 是不是有渐变
					boolean isGrad = true;
					if (grade == null) {
					...
					if (!isGrad) {
						radiusColor = a
								.getString(R.styleable.cri_criView_cri_normalcornersradiuscolor);
						if (radiusColor != null)
							grade.setColor(Color.parseColor(radiusColor));
					}
					strokeColor = a.getString(R.styleable.cri_criView_cri_strokecolors);
					if (strokeColor != null)
				...
				break;
			case R.styleable.cri_criView_cri_groundpress:

				GradientDrawable gradepress = null;
				// 渐变色
				gradColor = a
						.getString(R.styleable.cri_criView_cri_pressgradientcolors);
				if (gradColor != null) {
					split = gradColor.split("-");
					int colors[] = new int[split.length];
					for (int j = 0; j < split.length; j++) {
				...
				if (radiusInt != null) {
					...
					if (!isGrad) {
						radiusColor = a
								.getString(R.styleable.cri_criView_cri_presscornersradiuscolor);
						if (radiusColor != null)
				...
				break;
			case R.styleable.cri_criView_cri_groundforce:
				background = a.getDrawable(attr);
				drawable.addState(new int[] { android.R.attr.state_focused },
						background);
				break;
			case R.styleable.cri_criView_cri_groundselect:
